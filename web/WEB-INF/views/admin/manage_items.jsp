...
<c:choose>
    <c:when test="${requestScope['items'].size() gt 0}">
        <div class="container">
                ...
                <table class="main-table text-center table table-bordered">
                    ...
                    <c:forEach items="${requestScope['items']}" var="item">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href='/ecommerce/admin/items?action=Edit&itemid=${item.id}' class='btn btn-success'><i class='fa fa-edit'></i> Edit</a>
                                <a href='/ecommerce/admin/items?action=Delete&itemid=${item.id}' class='btn btn-danger confirm'><i class='fa fa-close'></i> Delete</a>
                                <c:if test="${item.approve eq 0}">
                                    <a href='/ecommerce/admin/items?action=Approve&itemid=${item.id}' class='btn btn-info activate'> <i class='fa fa-check'></i> Approve</a>
                                </c:if>
                            </td>
                        </tr>
                    </c:forEach>
            ...
            <a href="/ecommerce/admin/items?action=Add" class="btn btn-sm btn-primary">
